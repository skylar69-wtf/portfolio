function notifyMe(message, status, progress) {
      ...
      function check(cc2, separador, id) {
                    ...
                    xmlhttp.onreadystatechange = function()
                    {
                      ...
                      }  else if (xdata.match("Approved CVV")) {
                          document.getElementById("demo").innerHTML = 'CVV APPROVED !';
                          document.getElementById("testado").innerHTML = (eval(document.getElementById("testado").innerHTML) + 1);
                          document.getElementById("CVVLIVE").innerHTML = (eval(document.getElementById("CVVLIVE").innerHTML) + 1);
                          document.getElementById("cvv_conta").innerHTML = (eval(document.getElementById("cvv_conta").innerHTML) + 1);
                          $("#cvvapp").append(xdata);
                          rmLinha('#lista');
                          //notifyMe("Você tem mais 1 info aprovada.", "Transação autorizada com sucesso!");
                          //play();
